...
const navItems = [
...

  {
    ...
    name: "Seller",
    path: "/seller",
    subItems: [{ name: "Add Seller", path: "/add-seller" },
    { name: "Manange Seller", path: "/manage-seller", pro: false }
    ]
...
const AppSidebar = () => {
  ...
  const renderMenuItems = (items, menuType) => (
    <ul className="flex flex-col gap-4">
      {items.map((nav, index) => (
        ((nav.name === "Seller" && user.role == "vendor") ? "" :
          <li key={nav.name}>
            {nav.subItems ? (
              ...
              >
                <ul className="mt-2 space-y-1 ml-9">
                  {nav.subItems.map((subItem) => (
                    (user.role == "vendor" && ["Add Category", "Add Seller"].includes(subItem.name) ? "" :
                      <li key={subItem.name}>
                        <Link
                          // targeted
                          to={subItem.path}
